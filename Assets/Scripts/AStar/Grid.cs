...
public class Grid : MonoBehaviour {
	...
	void Awake() {
		//Gets the GridWorldSize according to the screen
		float worldHight = Camera.main.orthographicSize * 2f;
		float worldWidth = (worldHight / Screen.height) * Screen.width;
		gridWorldSize = new Vector2(worldWidth, worldHight);

		//Gets the grid size in integers
	...
	void UpgradeGrid()
	{
		...
		for (int x = 0; x < gridSizeX; x ++) 
		{
			...
			{
				Vector2 worldPoint = worldBottomLeft + Vector2.right * (x * nodeDiameter + nodeRadius) + Vector2.up * (y * nodeDiameter + nodeRadius);
				grid[x,y].walkable = (Physics2D.OverlapCircle(worldPoint,nodeRadius*3,unwalkableMask) == null); // if no collider2D is returned by overlap circle, then this node is walkable
